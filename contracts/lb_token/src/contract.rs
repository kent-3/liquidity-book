    ...
    let action = |old_perm: Permission,
     ...
     -> Permission {
        Permission {
            ...
            transfer_allowance_perm: match transfer {
                Some(i) => i,
                None => old_perm.transfer_allowance_perm,
            },
            transfer_allowance_exp: match transfer_expiry {
                Some(i) => i,
                None => old_perm.transfer_allowance_exp,
            },
        }
    };
...
fn impl_transfer(
    ...
    if from != &info.sender {
        ...
        match permission_op {
            ...
            Some(perm) if perm.transfer_allowance_exp.is_expired(&env.block) => {
                return Err(StdError::generic_err(format!(
                    "Allowance has expired: {}",
                    perm.transfer_allowance_exp
                )));
            }
            // not enough allowance to transfer amount
            Some(perm) if perm.transfer_allowance_perm < amount => {
                return Err(StdError::generic_err(format!(
                    "Insufficient transfer allowance: {}",
                    perm.transfer_allowance_perm
                )));
            }
            // success, so need to reduce allowance
            Some(mut perm) if perm.transfer_allowance_perm >= amount => {
                let new_allowance = perm
                    .transfer_allowance_perm
                    .checked_sub(amount)
                    .expect("something strange happened");
                perm.transfer_allowance_perm = new_allowance;
...
use lb_interfaces::lb_token::{QueryAnswer, QueryMsg, QueryWithPermit};
/////////////////////////////////////////////////////////////////////////////////
// Queries
/////////////////////////////////////////////////////////////////////////////////
