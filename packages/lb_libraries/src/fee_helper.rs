...
pub trait FeeHelper {
    ...
    fn verify_fee(&self) -> Result<(), FeeError>;
    ...
    fn get_composition_fee(&self, total_fee: u128) -> Result<u128, FeeError>;

    /// Calculates the protocol fee amount from the fee amount and the protocol share, rounding down.
    fn get_protocol_fee_amount(&self, protocol_share: u128) -> Result<u128, FeeError>;
}

/// Implementation of the FeeHelper trait for u128
impl FeeHelper for u128 {
    fn verify_fee(&self) -> Result<(), FeeError> {
        if *self > MAX_FEE {
    ...
    fn verify_protocol_share(&self) -> Result<(), FeeError> {
        if *self > MAX_PROTOCOL_SHARE as u128 {
    ...
    fn get_fee_amount(&self, total_fee: u128) -> Result<u128, FeeError> {
        ...
        let fee_amount = (U256::from(*self) * total_fee + denominator - 1) / denominator;
    ...
    fn get_composition_fee(&self, total_fee: u128) -> Result<u128, FeeError> {
        ...
        let composition_fee =
            U256::from(*self) * total_fee * (U256::from(total_fee) + PRECISION) / denominator;

        Ok(composition_fee.as_u128())
    }

    fn get_protocol_fee_amount(&self, protocol_share: u128) -> Result<u128, FeeError> {
        protocol_share.verify_protocol_share()?;

        let protocol_fee_amount = U256::from(*self) * protocol_share / BASIS_POINT_MAX as u128;
