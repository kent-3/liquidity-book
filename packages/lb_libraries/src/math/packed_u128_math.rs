...
// There are two unique types of Packed u128.
// 1. FixedPoint128x128 for representing price
// 2. AmountsPair for representing two different token amounts

/// A thin wrapper around a `Bytes32` representing a 128.128 fixed point number.
pub struct FixedPoint128x128(pub Bytes32);

impl AsRef<[u8]> for FixedPoint128x128 {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl From<[u8; 32]> for FixedPoint128x128 {
    fn from(bytes: [u8; 32]) -> Self {
        FixedPoint128x128(bytes)
    }
}

impl PackedUint128Math for FixedPoint128x128 {}

/// A thin wrapper around a `Bytes32` representing a pair of token X and token Y amounts.
pub struct AmountsPair(pub Bytes32);

impl AsRef<[u8]> for AmountsPair {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl From<[u8; 32]> for AmountsPair {
    fn from(bytes: [u8; 32]) -> Self {
        AmountsPair(bytes)
    }
}

impl PackedUint128Math for AmountsPair {}
...
mod tests {
    ...
    fn test_lt() {
        ...
        assert!(<[u8; 32] as PackedMath>::lt(&bytes1, bytes2));
    ...
    fn test_gt() {
        ...
        let bytes1 = Bytes32::encode(25, 45);
        let bytes2 = Bytes32::encode(10, 20);
        assert!(<[u8; 32] as PackedMath>::gt(&bytes1, bytes2));
        // False Case
        let bytes1 = Bytes32::encode(10, 20);
        let bytes2 = Bytes32::encode(25, 45);
        assert!(!<[u8; 32] as PackedMath>::gt(&bytes1, bytes2));
