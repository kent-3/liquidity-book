...
/// Computes (x + y) % k where the addition is performed with arbitrary
/// precision and does not wrap around at 2^256.
...
pub trait U256x256Math {
    fn mul_div_round_down(&self, y: U256, denominator: U256) -> Result<U256, U256x256MathError>;
    fn mul_div_round_up(&self, y: U256, denominator: U256) -> Result<U256, U256x256MathError>;
    fn mul_shift_round_down(&self, y: U256, offset: u8) -> Result<U256, U256x256MathError>;
    fn mul_shift_round_up(&self, y: U256, offset: u8) -> Result<U256, U256x256MathError>;
    fn shift_div_round_down(
        &self,
        offset: u8,
        denominator: U256,
    ) -> Result<U256, U256x256MathError>;
    fn shift_div_round_up(&self, offset: u8, denominator: U256) -> Result<U256, U256x256MathError>;
    fn _get_mul_prods(x: U256, y: U256) -> Result<(U256, U256), U256x256MathError>;
    fn _get_end_of_div_round_down(
        x: U256,
        y: U256,
        denominator: U256,
        prod0: U256,
...
impl U256x256Math for U256 {
    ...
    fn mul_div_round_down(&self, y: U256, denominator: U256) -> Result<U256, U256x256MathError> {
        ...
        if *self == y && y == denominator {
            return Ok(*self);
        }

        let (prod0, prod1) = Self::_get_mul_prods(*self, y)?;

        let result = Self::_get_end_of_div_round_down(*self, y, denominator, prod0, prod1)?;
    ...
    /// * `result` - The result as U256
    fn mul_div_round_up(&self, y: U256, denominator: U256) -> Result<U256, U256x256MathError> {
        let mut result = Self::mul_div_round_down(self, y, denominator)?;

        if mulmod(*self, y, denominator) != 0 {
            result += 1;
        }
    ...
    fn mul_shift_round_down(&self, y: U256, offset: u8) -> Result<U256, U256x256MathError> {
        ...
        if prod1 != 0 {
            ...
            result += prod1 << (256 - offset as u16);
        }
    ...
    /// * `result` - The result as U256
    fn mul_shift_round_up(&self, y: U256, offset: u8) -> Result<U256, U256x256MathError> {
        let mut result = Self::mul_shift_round_down(self, y, offset)?;

        if mulmod(*self, y, U256::ONE << offset) != 0 {
            result += 1;
        }
    ...
    fn shift_div_round_down(
        ...
        let prod0 = self << offset; // Least significant 256 bits of the product
        let prod1 = self >> (256 - offset as u16); // Most significant 256 bits of the product

        Self::_get_end_of_div_round_down(*self, U256::ONE << offset, denominator, prod0, prod1)
    }
    ...
    /// * `result` - The result as U256
    fn shift_div_round_up(&self, offset: u8, denominator: U256) -> Result<U256, U256x256MathError> {
        let mut result = Self::shift_div_round_down(self, offset, denominator)?;

        if mulmod(*self, U256::ONE << offset, denominator) != 0 {
            result += 1;
        }
...
pub fn _get_end_of_div_round_down(
    ...
    if prod1 == 0 {
        ...
        // in modular arithmetic, doubling the correct bits in each step
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^8
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^16
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^32
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^64
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^128
        inverse =
            inverse.wrapping_mul(U256::from(2u8).wrapping_sub(denominator.wrapping_mul(inverse))); // inverse mod 2^256
...
//     }
//
//     fn mulmod_2(a: Uint256, b: Uint256, modulus: Uint256) -> Uint256 {
//         if modulus.is_zero() {
//             return Uint256::zero();
//         }
//         ((a.full_mul(b)) % Uint512::from(modulus))
//             .try_into()
//             .expect("this should always work")
//     }
//     pub fn mul_div_round_up_2(
//         x: Uint256,
//         y: Uint256,
//         denominator: Uint256,
//     ) -> Result<Uint256, U256x256MathError> {
//         let mut result = x.multiply_ratio(y, denominator);
//
//         if Self::mulmod_2(x, y, denominator) != Uint256::zero() {
//             result += Uint256::one();
//         }
//
//         Ok(result)
...
mod tests {
    ...
    use crate::libraries::constants::{PRECISION, SCALE_OFFSET};
    use ethnum::U256;
    use std::str::FromStr;
    ...
    fn test_get_mul_prods() {
        ...
        let y =
            U256::from_str_prefixed("42008768997448919173843294709597899956404323600000").unwrap();
        let mul_prods = <U256 as U256x256Math>::_get_mul_prods(x, y).unwrap();
    ...
    fn test_shift_div_round_down() {
        ...
        let shifted = x << shift;
        let (prod0, prod1) = <U256 as U256x256Math>::_get_mul_prods(shifted, U256::ONE).unwrap();
